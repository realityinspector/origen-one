...
test.describe('Parent & Learner Workflows', () => {
  ...
  test('8. Generate lesson and validate SVG rendering', async () => {
    ...
    // Create lesson via API with exponential backoff — production AI can return 503 under load
    const maxRetries = 5;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      console.log(`Attempt ${attempt + 1}: status=${result.status}, data=${JSON.stringify(result.data).substring(0, 200)}`);
    ...
    if (!lessonId) {
      console.log('SKIP: AI service unavailable — lesson generation failed after all retries');
      test.skip();
      return;
    }
  ...
  test('9. Quiz flow with question validation', async () => {
    if (!lessonId) {
      console.log('SKIP: No lesson available (AI service unavailable)');
      test.skip();
      return;
    }
