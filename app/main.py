...
async def run_migrations() -> dict:
    ...
    from app.middleware.auth import _is_dev_bypass_enabled
    if not _is_dev_bypass_enabled():
        from fastapi import HTTPException
        raise HTTPException(status_code=403, detail="Admin endpoints require dev bypass")
    ...
    async with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute("CREATE INDEX IF NOT EXISTS idx_conv_msg_conversation ON conversation_messages (conversation_id);")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_conv_msg_created ON conversation_messages (conversation_id, created_at DESC);")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_conv_msg_role ON conversation_messages (conversation_id, role);")
        conn.commit()
...
async def seed_test_data() -> dict:
    """Seed test data for smoke testing (dev only)."""
    from app.middleware.auth import _is_dev_bypass_enabled
    if not _is_dev_bypass_enabled():
        from fastapi import HTTPException
        raise HTTPException(status_code=403, detail="Admin endpoints require dev bypass")
...
async def db_check() -> dict:
    ...
    from app.db import get_connection
    result = {"database": "unknown", "age": "unknown", "graph": "unknown"}
    try:
        async with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT * FROM cypher('sunschool_graph', $$ MATCH (n) RETURN count(n) $$) AS (count agtype);
                """)
