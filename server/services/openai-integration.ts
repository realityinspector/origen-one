...
// Types for enhanced lesson content
...
export async function generateFallbackSvgImage(
  ...
  try {
    ...
    const svgContent = await chat(messages, {
      max_tokens: 1500,
      temperature: 0.3
    });
 ...
 */
function createFallbackDiagram(topic: string, diagramType: string): string {
 ...
 */
function createCycleDiagram(topic: string): string {
 ...
 */
function createFlowchartDiagram(topic: string): string {
 ...
 */
function createComparisonDiagram(topic: string): string {
