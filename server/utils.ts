...
// Function to generate a basic AI lesson based on grade level and topic
export function generateLesson(gradeLevel: number, topic?: string): InsertLesson['spec'] {
  // This would typically call an AI service, but we'll provide a simple template for the MVP
  ...
  const safeGradeLevel = gradeLevel >= 1 && gradeLevel <= 5 ? gradeLevel : 3;
  ...
  const lessonContent = `
    ...
    ${getContentForTopic(selectedTopic, safeGradeLevel)}
  `;
  
  // Generate quiz questions
  const questions = generateQuizQuestions(selectedTopic, safeGradeLevel);
...
function getContentForTopic(topic: string, gradeLevel: number): string {
...
function generateQuizQuestions(topic: string, gradeLevel: number) {
  // In a real implementation, these would be generated by an AI service based on the lesson content
  const questions: {
    text: string;
    options: string[];
    correctIndex: number;
