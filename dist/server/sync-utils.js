...
exports.synchronizeToExternalDatabase = synchronizeToExternalDatabase;
 ...
 */
async function synchronizeToExternalDatabase(parentId, syncConfig) {
    ...
    try {
        ...
        const [parent, children, profiles, allLessons, allAchievements] = await Promise.all([
            // Get the parent user
            storage_1.storage.getUser(parentId),
            // Get all child users (learners) of this parent
            storage_1.storage.getUsersByParentId(parentId),
        ...
        if (childIds.length > 0) {
            ...
            const profilePromises = childIds.map(id => storage_1.storage.getLearnerProfile(id));
            const resolvedProfiles = await Promise.all(profilePromises);
            const validProfiles = resolvedProfiles.filter((profile) => profile !== undefined);
            // Get lessons for all children
            const lessonPromises = childIds.map(id => storage_1.storage.getLessonHistory(id, 1000)); // Get up to 1000 lessons per child
            ...
            const achievementPromises = childIds.map(id => storage_1.storage.getAchievements(id));
        ...
        try {
            ...
            const updatedConfig = await storage_1.storage.updateSyncStatus(syncConfig.id, 'COMPLETED');
            if (updatedConfig) {
                ...
                const checkConfig = await db_1.db.query.dbSyncConfigs.findFirst({
                    where: (0, drizzle_orm_1.eq)(schema_1.dbSyncConfigs.id, syncConfig.id)
                });
                if (checkConfig) {
                    ...
                    const updateResult = await db_1.db.update(schema_1.dbSyncConfigs)
                        .set({
        ...
        try {
            ...
            // Use the storage API which provides better transaction handling
            const updatedConfig = await storage_1.storage.updateSyncStatus(syncConfig.id, 'FAILED', error.message || 'Synchronization failed');
            if (updatedConfig) {
 ...
 */
async function createOrUpdateSchema(client) {
    try {
 ...
 */
async function insertOrUpdateUser(client, user) {
    try {
 ...
 */
async function insertOrUpdateLearnerProfile(client, profile) {
    try {
        await client.query(`
 ...
 */
async function insertOrUpdateLesson(client, lesson) {
    try {
        await client.query(`
 ...
 */
async function insertOrUpdateAchievement(client, achievement) {
    try {
        await client.query(`
