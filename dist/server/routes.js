    ...
    return (req, res, next) => {
        ...
        (0, auth_2.authenticateJwt)(req, res, (err) => {
            if (err)
                return next(err);
            // Then check the role
            return (0, auth_2.hasRoleMiddleware)(roles)(req, res, next);
        });
    };
}
function registerRoutes(app) {
    ...
    app.get("/api/learners", hasRole(["PARENT", "ADMIN"]), (0, auth_2.asyncHandler)(async (req, res) => {
        ...
        if (req.user?.role === "ADMIN" && req.query.parentId) {
            learners = await storage_1.storage.getUsersByParentId(Number(req.query.parentId));
        }
    ...
    app.get("/api/learner-profile/:userId", isAuthenticated, (0, auth_2.asyncHandler)(async (req, res) => {
        ...
        if (req.user?.role === "ADMIN" ||
            (req.user?.role === "PARENT" && (await storage_1.storage.getUsersByParentId(req.user.id)).some(u => u.id === userId)) ||
            (req.user?.id === userId)) {
            try {
    ...
    app.post("/api/lessons/create", isAuthenticated, (0, auth_2.asyncHandler)(async (req, res) => {
        ...
        try {
            ...
            const lessonSpec = await (0, utils_1.generateLesson)(gradeLevel, topic);
            // Create the lesson
    ...
    app.post("/api/lessons/:lessonId/answer", isAuthenticated, (0, auth_2.asyncHandler)(async (req, res) => {
        ...
        try {
            ...
            if (learnerProfile) {
                const lessonSpec = await (0, utils_1.generateLesson)(learnerProfile.gradeLevel);
