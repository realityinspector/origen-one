    ...
    return {
        pool: {
            ...
            query: async (sql, params) => {
                ...
                const client = await (await jest.requireActual('../../db').pool.connect());
...
describe('PointsService', () => {
    test('awards points and updates balance', async () => {
        ...
        const balance = await points_service_1.pointsService.getBalance(1);
        expect(balance).toBe(50);
    });
    test('history contains ledger entry', async () => {
        const history = await points_service_1.pointsService.getHistory(1);
        expect(history.length).toBeGreaterThan(0);
        expect(history[0].amount).toBeDefined();
    });
});
//# sourceMappingURL=points-service.test.js.map