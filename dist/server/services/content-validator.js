...
exports.validateQuestionForGrade = validateQuestionForGrade;
exports.validateLessonContent = validateLessonContent;
 ...
 */
function validateQuestionForGrade(question, gradeLevel) {
    const issues = [];
    ...
    if (gradeLevel <= 2) {
        ...
        const isYesNo = question.options.some(opt => opt.toLowerCase() === 'yes' || opt.toLowerCase() === 'no');
        const hasNumbers = question.options.every(opt => /^\d+$/.test(opt));
        const hasSimpleWords = question.options.every(opt => opt.split(/\s+/).length <= 2);
 ...
 */
function validateLessonContent(content, gradeLevel) {
    const issues = [];
    const recommendations = [];
 ...
 */
function validateQuizQuestions(questions, gradeLevel) {
    const allIssues = [];
 ...
 */
function generateValidationReport(result, contentType) {
    const lines = [];
