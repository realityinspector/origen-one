...
exports.updateConceptMastery = updateConceptMastery;
exports.updateMasteryFromQuiz = updateMasteryFromQuiz;
exports.getLearnerMastery = getLearnerMastery;
exports.getConceptsNeedingReinforcement = getConceptsNeedingReinforcement;
exports.getMasterySummary = getMasterySummary;
exports.bulkUpdateMasteryFromAnswers = bulkUpdateMasteryFromAnswers;
 ...
 */
async function updateConceptMastery(learnerId, conceptName, subject, isCorrect) {
    try {
        // Check if mastery record exists
        const existing = await db_1.db.execute((0, drizzle_orm_1.sql) `
 ...
 */
async function updateMasteryFromQuiz(learnerId, subject, conceptTags, isCorrect) {
 ...
 */
async function getLearnerMastery(learnerId, subject) {
    try {
        let query;
        if (subject) {
      ...
      `;
        ...
        return results.rows.map(row => ({
            ...
            lastTested: new Date(row.last_tested),
            needsReinforcement: row.needs_reinforcement,
 ...
 */
async function getConceptsNeedingReinforcement(learnerId, subject, limit = 5) {
    try {
        let query;
        if (subject) {
            query = (0, drizzle_orm_1.sql) `
 ...
 */
async function getMasterySummary(learnerId) {
    try {
 ...
 */
async function bulkUpdateMasteryFromAnswers(learnerId, subject, conceptsAndCorrectness) {
