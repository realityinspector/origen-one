...
exports.askBittensor = askBittensor;
 ...
 */
async function askBittensor(options) {
    ...
    `;
        const minersResponse = await axios_1.default.post(BITTENSOR_SUBNET_1_URL, {
            query: minersQuery
        }, {
            headers: {
        ...
        const selectedMiner = miners.reduce((prev, current) => (prev.stake > current.stake) ? prev : current);
        ...
        const response = await axios_1.default.post(inferenceUrl, inferencePayload, {
            headers: {
                'Authorization': `Bearer ${BITTENSOR_API_KEY}`,
                'Content-Type': 'application/json',
